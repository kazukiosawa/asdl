...
def parse_args():
    ...
    parser.add_argument('--log-path', default="/home/lfsm/code/asdfghjkl/result/resnet50", type=str)
...
class Metric(object):
    ...
    def update(self, n, loss, outputs, targets):
        with torch.inference_mode():
            self._n += n
            self._loss += loss * n ### why loss time n
...
def train(epoch, dataset, model, criterion, opt, kfac, args):
    ...
    for i, (inputs, targets) in enumerate(dataset.loader):
        ...
        dummy_y = kfac.setup_model_call(model, inputs)
        loss_func = torch.nn.CrossEntropyLoss(label_smoothing=args.label_smoothing)
        kfac.setup_loss_call(loss_func, dummy_y, targets)
        if args.cov_update_freq != -1 and i % args.cov_update_freq == 0:
            outputs,loss = kfac.forward_and_backward(accumulate=True)
        else:
        ...
        if i % 100 == 0:
            writer.add_scalar("Loss/train_iter100", metric.loss, epoch*len(dataset.loader)+i)
            # print(f'Epoch {epoch} {i}/{len(dataset.loader)} Train {metric}')
    print(f'Epoch {epoch} Train {metric} LR: {lr}')
    return epoch,metric,lr
...
if __name__ == '__main__':
    ...
    kfac = KfacGradientMaker(model,config)
    ...
    for e in range(args.epochs):
        epoch,metric,lr = train(e, dataset, model, criterion, opt, kfac, args)
        writer.add_scalar("Loss/train", metric.loss, epoch)
        torch.cuda.empty_cache()
        epoch,metric = test(e, dataset, model, criterion, args)
        writer.add_scalar("Loss/test", metric.loss, epoch)
        writer.add_scalar("Loss/train", metric.accuracy, epoch)
        torch.cuda.empty_cache()
        lr_scheduler.step()
