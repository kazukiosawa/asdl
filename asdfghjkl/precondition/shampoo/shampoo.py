...
from .matrix_functions import ComputePower
import numpy as np
import torch
...
class BlockPartitioner:
    ...
    (1024, 512) each.
  """
    def __init__(self, var, hps: ShampooHyperParams):
        ...
        for i, d in enumerate(var.shape):
            if hps.block_size > 0 and d > hps.block_size:
                ...
                indices = (np.arange(nsplit, dtype=np.int32) +
                           1) * hps.block_size
...
class Preconditioner:
    ...
    def __init__(self, var, hps):
        self._hps: ShampooHyperParams = hps
        ...
        if rank <= 1:
            ...
            self.statistics = [
                eps * torch.eye(s[0], device=device) for s in shapes
            ]
            self.preconditioners = [
    ...
    """
        ...
        for i, grad in enumerate(partitioned_grads):
            preconditioners_for_grad = self.preconditioners[i *
                                                            num_splits:(i +
                                                                        1) *
                                                            num_splits]
            rank = len(grad.shape)
            ...
            for j in range(rank):
                ...
                precond_grad = torch.tensordot(precond_grad, preconditioner,
                                               [[0], [0]])
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if self.hps.interval_scheduler is not None:
                    ...
                    update_preconditioning_compute_steps = update_statistics_TF
                else:
                    update_statistics_TF = state[STEP] % hps.statistics_compute_steps == 0
                    update_preconditioning_compute_steps = state[STEP] % hps.preconditioning_compute_steps == 0

                if update_statistics_TF:
                    preconditioner.add_statistics(grad)
                if update_preconditioning_compute_steps:
