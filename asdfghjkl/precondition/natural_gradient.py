...
class NaturalGradient:
    ...
    def precondition_module(self, module, shape=None, vectors: ParamVector = None,
        ...
        if vectors is not None:
            if vit_check(module):
                vec_weight = vectors.get_vector_by_param(module.cls_token, None)
                vec_bias = vectors.get_vector_by_param(module.position_embeddings, None)
            else:
        ...
        if vec_weight is None:
            if vit_check(module):
                vec_weight = module.cls_token.grad
            elif module.weight.requires_grad:
                vec_weight = module.weight.grad
        if vec_bias is None:
            if vit_check(module):
                vec_bias = module.position_embeddings.grad
            elif _bias_requires_grad(module):
