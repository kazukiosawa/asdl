...
from .operations import OP_BATCH_GRADS, OP_BATCH_GRADS_AUG
...
def batch_aug_gradient(model, closure, return_outputs=False, kron_jac=False):
    with extend(model, OP_BATCH_GRADS_AUG if kron_jac else OP_BATCH_GRADS) as cxt:
        outputs = closure()
        grads = []
        for module in model.modules():
            g = cxt.batch_grads(module, flatten=True)
