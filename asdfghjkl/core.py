        ...
        for module, _op_names in module_wise_assignments(model, *op_names, ignore_modules=ignore_modules, map_rule=map_rule):
            ...
            if has_fwd_op or has_bwd_op_with_inputs:
                if has_bwd_op_with_inputs:
                    def forward_hook(_module, in_data, out_data):
                        with stream_cxt:
                            cxt.call_operations_in_forward(_module, in_data[0], out_data)
                        if out_data.requires_grad:
                            def _backward_hook(out_grads):
                                with stream_cxt:
                                    cxt.call_operations_in_backward(_module, in_data[0], out_data, out_grads)
            ...
            if (not has_bwd_op_with_inputs) and has_bwd_op:
                def backward_hook(_module, unused, out_grads):
                    with stream_cxt:
                        try:
                            cxt.call_operations_in_backward(_module, None, None, out_grads[0])
                        except NameError:
